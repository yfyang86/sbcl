;;;; the WebAssembly definitions of some general purpose memory reference VOPs
...
;;; is one thread, so a plain read-modify-write is atomic enough.
(define-vop (set-instance-hashed)
  (:args (object :scs (descriptor-reg)))
  (:generator 5
    (load-reg object)
    (emit-store-word (- instance-pointer-lowtag)
      (load-reg object)
      (emit-load-word (- instance-pointer-lowtag))
      (inst i32.const (ash 1 stable-hash-required-flag))
      (inst i32.or))))
